...
def generate(batch_size, max_length, start_symbol, dictionary):
    ...
    gen_seq = []
    decoder_output = model.encoder.init_hidden(batch_size, model.encoder_hidden_size)
    start_symbol =  Variable(torch.LongTensor(1,1).fill_(start_symbol))
    if cuda:
        start_symbol = start_symbol.cuda()
    curr_input = model.encoded(start_symbol) # TODO: change here to look and handle batches
    for i in range(max_length):
        decoder_output = model.forward(curr_input, decoder_output)
...
def test_evaluate(data_source, data_order, test):
        ...
        with open(ref_path, 'w') as rp:
            with open(gen_path, 'w') as gp:
                ...
                for batch_num in data_order:
                    ...
                    gen_seq =  generate(batch_size, max_length, corpus.word_vocab.word2idx["<sos>"], corpus.word_vocab)
                    for r in ref_seq:
                        rp.write(r+" ")
                    for g in gen_seq:
                        gp.write(g+" ")
    ...
    os.system("echo \"************ Ununk files are empty dont trust them scores ************\"")
    return
