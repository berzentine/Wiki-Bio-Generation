...
def batchify(corpus, batchsize):
    ...
    for i in range(0,3):
        datum = sorted((data_store[i][4], data_store[i][0], data_store[i][1], data_store[i][2], data_store[i][3]), key=lambda tup: len(tup[0]))
        for d in range(0,len(datum), batchsize):
            batch = datum[d:d+batchsize] # create a batch
            table_max_length = -100
            for b in batch: # find biggest table
            ...
            for b in batch:
                ...
                """while(len(b[1])<table_max_length):
                    b[1].append(0) # value
                    b[2].append(0) # field
                    b[3].append(0) # ppos
                    b[4].append(0) # ppneg
