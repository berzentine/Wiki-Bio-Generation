...
from models.structure_aware import Seq2SeqDualModelSOTA
...
print("Load data")
corpus = data_reader.Corpus(data_path, vocab_path, alignment_path, alignment_pickle_path, use_pickle, 1, limit, verbose, use_alignments=False)
...
if use_alignments and alignment_function == 'timestep':
    print("SOTA + ALIGN + TIMESTEP MODEL")
    ...
    if not structure_aware:
        ...
        print("SOTA structure aware")
        model = Seq2SeqDualModelSOTA(sent_vocab_size=WORD_VOCAB_SIZE, field_vocab_size=FIELD_VOCAB_SIZE, ppos_vocab_size=POS_VOCAB_SIZE,
                                 pneg_vocab_size=POS_VOCAB_SIZE, value_vocab_size=WORD_VOCAB_SIZE, sent_embed_size=word_emb_size,
                                 field_embed_size=field_emb_size, value_embed_size=word_emb_size, ppos_embed_size=pos_emb_size,
                                 pneg_embed_size=pos_emb_size, encoder_hidden_size=hidden_size, decoder_hidden_size=hidden_size,
                                 decoder_num_layer=num_layers, verbose=verbose, cuda_var=cuda, x=x)
